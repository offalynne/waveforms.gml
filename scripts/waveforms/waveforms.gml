...
function         __waveform()
{
    ...
    TRIANGLE = function(_p, _f) { return (-2 / _f) * (_p - _f / 2 * floor((2 * _p)/ _f + 0.5)) * power(-1, floor((2 * _p)/ _f + 0.5)); }    
    SAWTOOTH = function(_p, _f) { return -(_p / _f - floor(0.5 + _p / _f)); };    
    SQUARE   = function(_p, _f) { return ((_p mod _f) < (_f / 2))? -0.5 : 0.5; };
    SINE     = function(_p, _f) { return sin(_p * 2 * pi / _f) / -2; };
