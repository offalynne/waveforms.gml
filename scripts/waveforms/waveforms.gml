...
function         __waveform()
{
    ...
    TRIANGLE = function(_phase, _frequency) { return (-2 / _frequency) * (_phase - _frequency / 2 * floor((2 * _phase)/ _frequency + 0.5)) * power(-1, floor((2 * _phase)/ _frequency + 0.5)); }    
    SAWTOOTH = function(_phase, _frequency) { return -(_phase / _frequency - floor(0.5 + _phase / _frequency)); };    
    SQUARE   = function(_phase, _frequency) { return ((_phase mod _frequency) < (_frequency / 2))? -0.5 : 0.5; };
    SINE     = function(_phase, _frequency) { return sin(_phase * 2 * pi / _frequency) / -2; };
